...
export class AgentRepository {
  ...
  create(agent: Agent): Agent {
    this.db
      .prepare(
        `INSERT INTO agents (id, worktree_id, name, status, model, effort, permission_mode, claude_session_id, pid, run_log_path, created_at, updated_at, archived_at, active_session_id, goal)
         VALUES (@id, @worktreeId, @name, @status, @model, @effort, @permissionMode, @claudeSessionId, @pid, @runLogPath, @createdAt, @updatedAt, @archivedAt, @activeSessionId, @goal)`,
      )
      .run({
        id: agent.id,
    ...
    this.db
      .prepare(
        `UPDATE agents SET name = @name, status = @status, model = @model, effort = @effort,
         ...
         active_session_id = @activeSessionId, goal = @goal
         WHERE id = @id`,
      )
      .run({
        id: agent.id,
