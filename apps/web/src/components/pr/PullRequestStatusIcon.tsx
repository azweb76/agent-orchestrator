...
import type { PullRequestStatusKind } from './pullRequestStatus';

/** GitHub octicon path data for each PR lifecycle status (16×16 viewBox). */
const STATUS_PATH: Record<PullRequestStatusKind, string> = {
...
function statusMuiColor(status: PullRequestStatusKind): SvgIconProps['color'] {
  switch (status) {
    case 'open':
      return 'success';
    case 'merged':
...
/**
 * GitHub-style pull-request glyph for open / draft / merged / closed.
 * Prefer these over colored status dots for PR lifecycle.
 */
...
} & Omit<SvgIconProps, 'color' | 'children'>) {
  return (
    <SvgIcon
      fontSize={fontSize}
      color={statusMuiColor(status)}
      viewBox="0 0 16 16"
      aria-hidden
      {...props}
    >
      <path fill="currentColor" d={STATUS_PATH[status]} />
    </SvgIcon>
  );
}
