...
Mutating tools (`create_agent_from_goal`, `archive_agent`, `dismiss_work_item`, `create_agent_task`, `update_agent_task`) require `confirm: true` after the user agrees. Use `list_agent_tasks` / `get_agent_task` before creating or updating a template.
